...
export class DialogParticipanteComponent {
  ...
  modificar() {
    if (!this.dataParticipante.valid) {
      const nombreControl = this.dataParticipante.get('nombre');
      const correoControl = this.dataParticipante.get('correo');

      if (nombreControl && nombreControl.hasError('required')) {
        this.toastrService.show("Ingrese un nombre para el usuario","Hay campos vacios o incorrectos", { status: "warning", destroyByClick: true, icon: "alert-triangle-outline" });
      }

      if (correoControl && correoControl.hasError('required')) {
        this.toastrService.show("Ingrese un correo electronico para el usuario","Hay campos vacios o incorrectos", { status: "warning", destroyByClick: true, icon: "alert-triangle-outline" });
      }

      if (correoControl && correoControl.hasError('email')) {
        this.toastrService.show("Ingrese un correo electronico valido para el usuario","Hay campos vacios o incorrectos", { status: "warning", destroyByClick: true, icon: "alert-triangle-outline" });
      }
